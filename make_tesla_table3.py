...
class insert_data:
    def inserting_data(self, data, me, ncs):
        dictionary = {}
        ...
        for key, value in me.items():
            print(key)
            print(value)
            dictionary.update({'mobile_enabled_' + key: value})
        '''
        '''
        tup = me.popitem()
        print(tup[0])
        ...
        for key, value in ncs.items():
            if isinstance(value, list):
                ncs_list = value
                if key is 'destination_charging':
                    count = 0
                    for ncs_dict in ncs_list:
                        count = count + 1
                        for k, v in ncs_dict.items():
                            if isinstance(v, dict):
                                for k2, v2 in v.items():
                                    k3 = 'destination_charging(' + count + ')_' + k + '_' + k2
                                    dictionary.update({k3: v2})
                            else:
                                k3 = 'destination_charging(' + count + ')_' + k
                                dictionary.update({k3: v})
                elif key is 'superchargers':
                    count = 0
                    for ncs_dict in ncs_list:
                        count = count + 1
                        for k, v in ncs_dict.items():
                            if isinstance(v, dict):
                                for k2, v2 in v.items():
                                    k3 = 'supercharger(' + count + ')_' + k + '_' + k2
                                    dictionary.update({k3: v2})
                            else:
                                k3 = 'supercharger(' + count + ')_' + k
                                dictionary.update({k3: v})
            else:
                if key is 'timestamp':
                    dt2 = datetime.datetime.fromtimestamp(value/1000)
                    dt2 = dt2 + datetime.timedelta(hours=9)
                    dictionary.update({'nearby_charging_sites_timestamp': dt2})
                '''
                else:
                    dt3 = datetime.datetime.fromtimestamp(value)
                    print(dt3)
                    dt3 = dt3 + datetime.timedelta(hours=9)
                    print(dt3)
                    dictionary.update({key: dt3})
                '''
                    
        

        pprint(dictionary)
        #tesla.create(**dictionary)
...
if __name__ == '__main__':
    ...
    #pprint(tesla_api.vehicle_data())
    me = tesla_api.mobile_enabled()
    #pprint(tesla_api.mobile_enabled())
    ncs = tesla_api.nearby_charging_sites()
    #pprint(tesla_api.nearby_charging_sites())

    #db.create_tables([tesla])

    insert_data().inserting_data(data, me, ncs)
