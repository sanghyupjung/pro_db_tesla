...
#app = Celery('make_tesla_table9', backend='amqp://localhost//', broker='amqp://localhost//')

class Tesla_main(TeslaAPI, Database_tesla, Task):
    def __init__(self, email, password):
    #def __init__(self, email, password, func):
        #self.email = email
        #self.password = password
        #self.tesla_api = TeslaAPI(self.email, self.password)
        self.tesla_api = TeslaAPI(email, password)
        self.app = Celery('make_tesla_table9', backend='amqp://localhost//', broker='amqp://localhost//')
        ...
        self.app.conf.update(
            CELERYBEAT_SCHEDULE = {
                'insert_data': {
                    #'task': 'make_tesla_table9.insert',
                    #'task': 'make_tesla_table9.Tesla_main.insert',
                    #'task': 'app.tasks.insert',
                    'task': 'make_tesla_table9.Tesla_main',
                    #'task': 'make_tesla_table9.run',n
                    'schedule': timedelta(minutes=1),
                    'options': {
                        'expires': 2*60, # 2 minutes
                    },
                    'args': (self.tesla_api.vehicle_data()['vehicle_id'], 1) # vehicle_id : 1124037660
                }
            }
        )
    ...
    #@app.task()
    #def insert(self, v_id, a):
    def run(self, v_id, a):
        try:
            data = self.tesla_api.vehicle_data()
            #Database_tesla(str(v_id))
...
#class func:
    def __init__(self, email, password):
        ...
        #app = self.app
        #app.register_task(t)
        #self.app.register_task(self.work(email, password))
        self.app.register_task(self.work())
        #self.app.register_task(func(self.email, self.password).work())
        #app.register_task(self.work())
    #def work(self, email, password):
    def work(self):
        #self.app.register_task()
        t = Tesla_main(self.email, self.password)
        #t = Tesla_main(email, password)
        return t
        #pass
...
if __name__ == '__main__':
    ...
    while(1):
        ...
        if x == 1:
            ...
            t = Tesla_main(email, password)
            app = t.app
            #app.register_task(t.insert(t.tesla_api.vehicle_data()['vehicle_id'], 1))
            #app.register_task(app.tasks.insert(t.tesla_api.vehicle_data()['vehicle_id'], 1))
            app.register_task(t)
            #app.register_task(Tesla_main(email, password).insert(1124037660, 1))
            #t.register()
            #Tesla_main(email, password)
            #t.delay(1124037660, 1)
            print("received email and password")
        elif x == 2:
            os.system("celery -A make_tesla_table9 worker -B -l info")
            #os.system("celery multi start 3 -A make_tesla_table9 -B -l info")
        elif x == 3:
            os.system("celery status")
        elif x == 4:
            os.system("celery multi stopwait 1 -A test_tesla3 -B -l info")
        elif x == 5:
            print('good bye')
            exit()
        else:
