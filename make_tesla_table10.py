...
class tesla_main(TeslaAPI, database_tesla, Task):
    def __init__(self, email, password):
        ...
        #self.app = Celery('make_tesla_table10', backend='amqp://localhost//', broker='amqp://localhost//')
...
#app = Celery('make_tesla_table10', backend='amqp://localhost//', broker='amqp://localhost//')
    ...
    while(1):
        ...
        if x == 1:
            ...
            app.conf.update(
                CELERYBEAT_SCHEDULE = {
                    'insert_data': {
                        'task': 'make_tesla_table10.tesla_main',
                        'schedule': timedelta(minutes=1),
                        'options': {
                            'expires': 2*60, # 2 minutes
                        },
                        'args': (t.tesla_api.vehicle_data()['vehicle_id'], 1) # vehicle_id : 1124037660
                    }
                }
            )

            app.register_task(t)
            #app.register_task(tesla_main(email, password))

            print("received email and password")
        elif x == 2:
            os.system("celery -A make_tesla_table10 worker -B -l info")
            #os.system("celery multi start 1 -A make_tesla_table10 -B -l info")
        elif x == 3:
