...
class CeleryConfig:
    CELERYBEAT_SCHEDULE = {
        'add-after-every-minute': {
            'task': 'make_tesla_table8.insert_tesla_celery',
...
'''
#app = Celery('make_tesla_table8', backend='amqp://localhost//', broker='amqp://localhost//')
...
'''
def make_tesla_table_with_celery(email, password, t):
    t.tesla_main()
'''


app = Celery('make_tesla_table8', backend='amqp://localhost//', broker='amqp://localhost//')
app.config_from_object(tesla_main)

@app.task()
def insert_tesla_celery(email, password, t):
    t.tesla_main()

#@app.task()
#def insert_tesla_celery(t):
    #make_tesla_table_with_celery(email, password, t)
    #t.tesla_main()
...
if __name__ == '__main__':
    ...
    while(1):
        ...
        if x == 1:
            ...
            #password = input('password : ')
            t.tesla_main()
            #result = insert_tesla_celery.delay(email, password, t)
            #result = insert_tesla_celery.delay(t)
        elif x == 3:
            #print(result.status)
            print('status')
        elif x == 4:
