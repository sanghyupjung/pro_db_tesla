...
class database_tesla():
    def __init__(self, v_id):
        ...
        class timestamp(BaseModel):
            timestamp = DateTimeField(default=None)
            class Meta:
                table_name = "test_" + v_id + "_timestamp"
        '''

        #db.create_tables([tesla, idle, charge_state, climate_state, drive_state, gui_settings, vehicle_state, vehicle_config, timestamp])
    ...
    def inserting_data(self, data):
        ...
        #self.timestamp_dict = self.change_to_each_table(self.timestamp_list)
...
def record_error(e):
    try:
        ...
        record = []
    record.append(e)
    with open('error_about_tesla.txt', 'wb+') as fp:
        pickle.dump(record, fp)



app = Celery('tesla', backend='amqp://localhost//', broker='amqp://localhost//')
...
def tesla_main():
    while(1):
        ...
        if x == 1:
            ...
            tesla_api = TeslaAPI(email, password)
            vehicle_id = tesla_api.vehicle_data()['vehicle_id']
            app.conf.update(
                CELERYBEAT_SCHEDULE = {
                    'insert_data': {
                        'task': 'tesla.insert',
                        'schedule': timedelta(minutes=1),
                        'args': (vehicle_id, 1) # vehicle_id : 1124037660
                    }
                }
            )
            ...
            #os.system("celery multi start 1 -A tesla -B -l info")
            os.system("celery -A tesla worker -B -l info")

        elif x == 3:
            os.system("celery status")

        elif x == 4:
            os.system("celery multi stopwait 1 -A tesla -B -l info")
